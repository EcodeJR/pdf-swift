...
const sendWelcomeEmail = async (email, name) => {
  const html = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      <h1 style="color: #3b82f6;">Welcome to PDF Swift!</h1>
      <p>Hi there,</p>
      ...
      <p>Happy converting!<br>The PDF Swift Team</p>
    </div>
  `;

  try {
    await sendEmail(email, 'Welcome to PDF Swift!', html);
  } catch (error) {
...
const sendSecurityAlert = async (email, ip, device) => {
  const html = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      ...
      <p>We detected a new login to your PDF Swift account:</p>
      <ul>
      ...
      <p>Best regards,<br>PDF Swift Security Team</p>
    </div>
  `;

  try {
    await sendEmail(email, 'New Login Detected - PDF Swift', html);
  } catch (error) {
...
const sendPasswordReset = async (email, resetToken) => {
  ...
  const html = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      ...
      <p>You requested to reset your password for your PDF Swift account.</p>
  ...
  try {
    await sendEmail(email, 'Password Reset Request - PDF Swift', html);
  } catch (error) {
...
const sendPasswordChanged = async (email) => {
  const html = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      ...
      <p>Best regards,<br>PDF Swift Team</p>
    </div>
  `;

  try {
    await sendEmail(email, 'Password Changed Successfully - PDF Swift', html);
  } catch (error) {
...
const sendSubscriptionConfirmation = async (email) => {
  const html = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      ...
      <p>Thank you for upgrading to PDF Swift Premium!</p>
      ...
      <p>Thank you for your support!<br>The PDF Swift Team</p>
    </div>
  `;

  try {
    await sendEmail(email, 'Welcome to Premium! - PDF Swift', html);
  } catch (error) {
