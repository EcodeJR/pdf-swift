...
const guestRateLimiter = rateLimit({
  ...
  message: { 
    message: 'You have exceeded the 3 conversions per hour limit. Please create a free account for cloud storage or upgrade to Premium for unlimited conversions.',
    limitExceeded: true
  },
...
const checkUserConversionLimit = async (req, res, next) => {
  try {
    ...
    if (req.user) {
      ...
      if (user.conversionsThisHour >= parseInt(process.env.FREE_CONVERSIONS_PER_HOUR || 3)) {
